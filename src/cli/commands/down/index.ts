...
export async function down(prisma: PrismaClient, steps: number = 1, config?: MigrationsConfig, interactive?: boolean) {
...
export async function promptDownMode(): Promise<string> {
  const choices = [
    ...
    { name: chalk.blue('Select specific migration to rollback to'), value: 'specific' },
...
export async function runRollbackForMode(mode: string, migrations: Migrations, applied: MigrationFile[]): Promise<number> {
...
export async function rollbackSteps(migrations: Migrations, applied: MigrationFile[]): Promise<number> {
...
export async function rollbackToSpecific(migrations: Migrations, applied: MigrationFile[]): Promise<number> {
