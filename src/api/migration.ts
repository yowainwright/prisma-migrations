...
export interface PrismaClientLike {
  ...
  $queryRaw?<T = unknown>(sql: TemplateStringsArray | string, ...values: unknown[]): Promise<T>;
}
...
export interface MigrationContext<TPrisma = PrismaClientLike> {
  ...
  sql: <T = unknown>(query: TemplateStringsArray | string, ...values: unknown[]) => Promise<T>;

  execute: (query: TemplateStringsArray | string, ...values: unknown[]) => Promise<number>;

  createTable: (tableName: string, definition: string) => string;

  dropTable: (tableName: string, ifExists?: boolean) => string;

  addColumn: (tableName: string, columnName: string, definition: string) => string;

  dropColumn: (tableName: string, columnName: string) => string;

  createIndex: (indexName: string, tableName: string, columns: string | string[], unique?: boolean) => string;

  dropIndex: (indexName: string, tableName?: string) => string;
}

export type MigrationFunction<TPrisma = PrismaClientLike> = (context: MigrationContext<TPrisma>) => Promise<void>;
...
export function createMigrationContext<TPrisma extends PrismaClientLike = PrismaClientLike>(
  prisma: TPrisma
): MigrationContext<TPrisma> {
  return {
    prisma,

    sql: async <T = unknown>(query: TemplateStringsArray | string, ...values: unknown[]): Promise<T> => {
    ...
    execute: async (query: TemplateStringsArray | string, ...values: unknown[]): Promise<number> => {
      if (typeof query === 'string') {
    ...
    createIndex: (indexName: string, tableName: string, columns: string | string[], unique: boolean = false): string => {
