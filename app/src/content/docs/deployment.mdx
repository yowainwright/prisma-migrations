...
Locks are renewable, owner-scoped row leases. `lockLeaseDuration` controls recovery time after a process exits without releasing its lease.

## Transaction Behavior

Each migration runs inside a Prisma transaction. Database DDL rules still apply: MySQL implicitly commits many schema statements, so a failed migration can leave partial schema changes. Inspect the database before retrying any failed migration.
