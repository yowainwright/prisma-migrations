...
This creates `migration.sql` and `down.sql` in a new timestamped directory.

Existing Prisma migrations may not have rollback files. New migrations include `down.sql`, so they can be rolled back safely.
