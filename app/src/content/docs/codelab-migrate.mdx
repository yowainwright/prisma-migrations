...
Create a new directory for prisma-migrations (separate from Prisma's migrations):

```bash
mkdir migrations
```

**Important**: Keep your existing `prisma/migrations` directory intact. The new `migrations` directory is for future migrations with rollback support.
...
This tells prisma-migrations where to find new migration files.
...
This creates: `migrations/[timestamp]_add_comments_table/migration.ts`
...
1. Keep existing Prisma migrations
2. Use prisma-migrations for new migrations only
...
prisma-migrations can only rollback migrations with `down()` functions. Old Prisma migrations don't have these, so they can't be rolled back.

Solution: Leave old migrations alone, only rollback new ones.
...
- Existing Prisma migrations untouched
- New migrations have rollback support
