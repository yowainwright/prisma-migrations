...
describe('Migrations', () => {
  ...
  beforeEach(() => {
    ...
    migrations = new Migrations(mockPrisma, { migrationsDir: testMigrationsDir });
  });
  ...
  const createMigration = (id: string, name: string, upFn = '', downFn = '') => {
    const migrationDir = join(testMigrationsDir, `${id}_${name}`);
    mkdirSync(migrationDir, { recursive: true });
    writeFileSync(
      join(migrationDir, 'migration.ts'),
      `
        export async function up(prisma) { ${upFn} }
        export async function down(prisma) { ${downFn} }
      `
    );
  };
  ...
  describe('constructor', () => {
    ...
    test('should use custom migrations directory from config', () => {
      const m = new Migrations(mockPrisma, { migrationsDir: './custom/migrations' });
      expect(m).toBeDefined();
    });
  });
  ...
  describe('up', () => {
    test('should run all pending migrations', async () => {
      createMigration('001', 'first', 'await prisma.$executeRaw`SELECT 1`');
      createMigration('002', 'second', 'await prisma.$executeRaw`SELECT 2`');
  ...
  describe('down', () => {
    test('should rollback specified number of migrations', async () => {
      createMigration('001', 'first', '', 'await prisma.$executeRaw`DROP TABLE test`');

      mockPrisma.$queryRaw = mock(() =>
        Promise.resolve([{ id: '001' }])
      );
    ...
    test('should throw error if migration file not found', async () => {
      ...
      await expect(migrations.down(1)).rejects.toThrow('Migration file not found for 001');
    });
  });
