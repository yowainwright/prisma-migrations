...
describe('End-to-End Migration Tests', () => {
  ...
  test('should create and run migrations like Knex.js', async () => {
    ...
    console.log('Initial status:', status.map(s => `${s.name}: ${s.status}`));
    ...
    assert.strictEqual(upResult.success, true, 'Migrations should run successfully');
    assert.strictEqual(upResult.migrations.length, 3, 'Should apply 3 migrations');

    console.log('✓ Applied migrations:', upResult.migrations.map(m => m.name));

    // 6. Check status after running up - should show 3 applied migrations
    status = await manager.getMigrationStatus();
    console.log('Status after up:', status.map(s => `${s.name}: ${s.status}`));
    ...
    assert.strictEqual(rollbackResult.migrations.length, 1, 'Should rollback 1 migration');
    assert.strictEqual(rollbackResult.migrations[0].name, 'add_user_email_index');
    ...
    try {
      ...
      const usersTable = await dbAdapter.prisma.$queryRaw`
        ...
        WHERE table_schema = 'public' AND table_name = 'users'
      ` as any[];
      assert.strictEqual(usersTable.length, 1, 'Users table should still exist');

      const postsTable = await dbAdapter.prisma.$queryRaw`
        SELECT table_name FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_name = 'posts'
      ` as any[];
      assert.strictEqual(postsTable.length, 1, 'Posts table should still exist');
      ...
      const indexExists = await dbAdapter.prisma.$queryRaw`
        ...
        WHERE tablename = 'users' AND indexname = 'idx_users_email'
      ` as any[];
      assert.strictEqual(indexExists.length, 0, 'Email index should be removed');
    ...
    assert.strictEqual(rollbackToResult.success, true, 'Rollback to specific migration should succeed');
    assert.strictEqual(rollbackToResult.migrations.length, 1, 'Should rollback 1 more migration');
    assert.strictEqual(rollbackToResult.migrations[0].name, 'create_posts_table');
    ...
    try {
      ...
      console.log('✓ Rollback to specific migration verified - only users table exists');
    } finally {
      await manager.destroy();
    }
    ...
    const fullRollbackResult = await manager.rollbackMigrations({ 
      to: 'initial' 
    });
    ...
    assert.strictEqual(finalRollbackResult.success, true, 'Final rollback should succeed');
    ...
    try {
      ...
      ` as any[];
      assert.strictEqual(tables.length, 0, 'No application tables should exist');
    ...
    assert.strictEqual(dryRunResult.migrations.length, 3, 'Dry run should show 3 migrations');
  ...
  test('should handle migration errors gracefully', async () => {
    ...
    assert.strictEqual(applied, 0, 'No migrations should be applied after failure');
